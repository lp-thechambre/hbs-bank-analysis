...
Eastmoney API parameters are stable (unchanged for years). Scripts use
hardcoded defaults as the primary data source. The optional api_profile.json
serves as an override mechanism — only needed when the API actually changes.

Load and merge: hardcoded defaults (primary) < api_profile.json (override).
"""
...
def load_api_profile(profile_path):
    ...
    Also warns if the profile is very old (age > 90 days), but does NOT
    block execution — hardcoded defaults are the primary data source.
    """
    try:
    ...
    days = profile.get("expires_after_days", 90)
    if discovered:
        try:
            dt = datetime.fromisoformat(discovered)
            age = (datetime.now() - dt).days
            if age > days:
                print(f"Note: profile is {age}d old. If pipeline fails, "
                      f"consider manually refreshing api_profile.json.", file=sys.stderr)
