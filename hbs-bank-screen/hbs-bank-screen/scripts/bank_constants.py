...
BANK_LIST = [
    ...
    {"code": "SH601825", "name": "沪农商行", "group": "rural_commercial"},
    ...
    {"code": "SH600919", "name": "江苏银行", "group": "city_commercial"},
...
def normalize_secuocode(code):
    """Normalize SECUCODE to canonical SHxxxxxx/SZxxxxxx format.

    Handles both Eastmoney API format (601398.SH) and internal format (SH601398).
    """
    if not code:
        return code
    # Already canonical: SH601398 or SZ000001
    if (code.startswith("SH") or code.startswith("SZ")) and "." not in code:
        return code
    # API format: 601398.SH or 000001.SZ
    if "." in code:
        parts = code.split(".")
        if len(parts) == 2:
            ticker, exchange = parts
            return exchange.upper() + ticker
    return code


def code_to_secid(code):
    """Convert SH601398 -> 1.601398, SZ000001 -> 0.000001"""
    code = normalize_secuocode(code)
