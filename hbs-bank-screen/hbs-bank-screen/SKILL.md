...
5. Main session spawns Synthesis (1 `sessions_spawn`)
...
### Step 5: Layer 3 — Synthesis (1 Spawn)

**Before spawning synthesis**, verify that qualitative layer outputs exist:
...
If user confirms or qual files exist, launch **one synthesis spawn** via `sessions_spawn`:
...
### Step 6: Compile Analysis Trail (Main Session)
...
### Step 7: Display Results (Main Session)
