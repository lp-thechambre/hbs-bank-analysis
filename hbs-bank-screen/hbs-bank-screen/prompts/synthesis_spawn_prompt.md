...
### Step 0: Identify Macro Regime

Before analyzing banks, determine the current macro regime. Read `references/scoring_rules.md` § "Macro Weight Modifiers" for the regime table.

Based on the screening date and known macro conditions (rate trajectory, GDP trend, real estate stress, policy priorities), identify which regime(s) apply. Apply the corresponding weight adjustments:

- **Rate cut cycle**: Shift 5-10pts from D3 NIM sub-weight to D3 non-interest income sub-weights
- **Economic downturn**: Shift 5-10pts from D3 ROE/D4 growth to D2 PCR/D1 CET1
- **Real estate stress**: Systematic +5-10pt bonus on D2 for banks with low real estate exposure
- **Policy-driven**: Shift 5-10pts to D3/D4 for banks with high green/tech/inclusive loan share

Document the applied regime and weight adjustments in the final output under `macro_context`. If macro regime is unclear, use default weights.
...
For every RED and YELLOW bank that is NOT a final candidate, compile a structured rejection entry. This enables downstream Depth to auto-identify "threshold edge" banks:
...
"rejected_banks": [
  {
    "code": "SH600015",
    "name": "华夏银行",
    "reason": "R1: CET1=7.8% < 8.5% threshold",
    "rule": "R1",
    "metric": "CET1",
    "value": 7.8,
    "threshold": 8.5,
  ...
  },
  {
    "code": "SH601997",
    "name": "贵阳银行",
    "reason": "Quant REJECT (NPL 3.5%) + Qual REJECT (regional concentration risk)",
    "rule": "R2",
    "metric": "NPL",
    "value": 3.5,
    "threshold": 3.0,
    "layer": "L1_quant+L2_qual"
  }
]
```

Fields:
- `reason`: Human-readable explanation citing the specific rule and values
- `rule`: R1-R5 from scoring_rules.md hard thresholds, or "QUAL_REJECT" / "EDGE_REJECT" / "SYNTHESIS_REJECT"
- `metric`: The specific metric that triggered rejection (null if qualitative)
- `value`: Actual metric value (null if unavailable or qualitative)
- `threshold`: The threshold value that was exceeded (null if qualitative)
- `layer`: Which layer(s) issued the rejection

Every rejected bank MUST have an entry. YELLOW banks that narrowly missed inclusion SHOULD have an entry — these are high-value signals for Depth's "revisit borderline" logic.

### Step 6: Build All-Banks Summary

Compile a summary array with all 42 banks for the main session. Every bank must include a `selection_path`:

- `consensus`: HIGH_CONFIDENCE_PASS — quant and qual both say PASS, no edge REJECT anomaly
- `conflict_resolved`: CONFLICT bank resolved in favor of inclusion
- `hard_reject`: UNANIMOUS_REJECT or hard threshold violated
- `borderline_inclusion`: Included only to meet the 10-candidate minimum

```json
"all_banks_summary": [
  {"code": "SH601398", "name": "工商银行", "tier": "green", "score": 78.5, "selection_path": "consensus", "brief_reason": "All layers PASS, strong capital + low NPL"},
  {"code": "SH601528", "name": "瑞丰银行", "tier": "yellow", "score": 55.2, "selection_path": "conflict_resolved", "conflict_detail": {"quant": "WATCH (NIM concern)", "qual": "PASS (regional growth offsets NIM)", "resolution": "qualitative override"}, "brief_reason": "Quant WATCH on NIM, qual PASS — qual reasoning accepted"},
  {"code": "SH600015", "name": "华夏银行", "tier": "red", "score": null, "selection_path": "hard_reject", "brief_reason": "UNANIMOUS_REJECT: NPL 3.2% exceeds hard threshold"}
]
```
...
- `selection_path` is REQUIRED for all 42 banks.
- `conflict_detail` is REQUIRED for all `conflict_resolved` and `borderline_inclusion` banks.

### Step 7: Add Source Tracking

For each final candidate, record which upstream markers support the inclusion, plus the selection path:

```json
"source_tracking": {
  "selection_path": "consensus",
...
If you overrode a marker (e.g., qual said WATCH but you decided PASS), set `selection_path` to `"conflict_resolved"` and note the override reason in `override`. Include `conflict_detail` showing all three positions (quant, qual, resolution).
...
- 冲突裁决: ARCHITECTURE-v2 裁判模式（Pattern A/B/C/D）
...
1. `{data_dir}/final_output.json` — Machine-readable JSON (format per `assets/output_schema.json`). Must include `all_banks_summary` (tier + selection_path for all 42 banks) and `rejected_banks` (structured rejection with rule/metric/value/threshold for every non-candidate).
...
Before any analysis, check which input files exist:
...
- Add `"missing_layers": ["L2_qual"]` to final_output.json
- In the screening report, clearly state: "L2 定性层未执行，本次裁决仅基于 L1 定量 + 边缘信号两层。建议重新执行定性层以获得完整的三层裁决。"
- Proceed with 2-layer synthesis, but downgrade confidence: mark ALL conflict resolutions as `"confidence": "degraded_no_qual"`
...
- The macro_context section is REQUIRED in final_output.json. If no regime applies, state "default_weights" with an explanation.
- Weight adjustments apply uniformly to ALL banks — this is a macro lens, not per-bank customization.
