...
from bank_constants import (
    ...
    get_type_override, code_to_secid,
)
...
def build_bank_index(records):
    """Build a dict keyed by SECUCODE from raw API records."""
    index = {}
    for rec in records:
...
def generate_index_csv(banks, classifications, scores, output_path):
    ...
    for code, bank in banks.items():
        ...
        sc = scores.get(code, {})
        pb_val = _compute_pb_for_csv(code, bank, None)  # simplified: no prices in index
        bps = bank.get("BPS")
...
def main():
    ...
    index_rows = generate_index_csv(banks, classifications, scores, index_path)
